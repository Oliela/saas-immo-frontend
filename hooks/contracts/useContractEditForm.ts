...
import type {
  ContractClause, CatalogClause, Client, Property,
  FormData, ContractType, ContractPayload,
} from "@/types/contractNew"
import type { ContractEditData } from "./useContractDetail"
import { paymentFrequencyMultiplier, paymentFrequencyLabel } from "@/data/clauseSysteme"
...
function injectTokens(
  ...
  financials: {
    ...
    cautionTotal: number; cautionMonths: number; rentAtSignature: number
  }
): string {
  const fmt = (n: number) => n ? `${n.toLocaleString("fr-FR")} FCFA` : "[MONTANT]"
  return content
...
export function useContractEditForm(initialData: ContractEditData) {
  ...
  const [selectedClient,   setSelectedClient]   = useState<Client | null>(initialData.selectedClient)
  const [selectedProperty, setSelectedProperty] = useState<Property | null>(initialData.selectedProperty)
  ...
  const financials = useMemo(() => {
    ...
    const rentMultiplier = paymentFrequencyMultiplier[formData.paymentFrequency] ?? 1
    ...
    return {
      rentAmount, depositAmount, cautionMonths, commissionRate,
      rentMultiplier, cautionTotal, commissionAmount, rentAtSignature, totalAtSignature,
    }
  ...
  const addInlineClause = (title: string, content: string) => {
    setClauses((prev) => [
      ...prev,
      { id: `inline-${Date.now()}`, title, content, type: contractType,
        source: "inline", isModified: false },
    ])
  }
  ...
  const updateClause = (updated: ContractClause) => {
    setClauses((prev) =>
      prev.map((c) => {
        ...
        return {
          ...updated,
          isModified: c.originalContent !== undefined
  ...
  const duplicateClause = (clause: ContractClause) => {
    const dupe: ContractClause = {
      ...clause, id: `inline-${Date.now()}`,
      title: `${clause.title} (Copie)`,
      source: "inline", clause_id: undefined,
      isModified: false, originalContent: undefined,
    }
    setClauses((prev) => {
  ...
  const clausesWithInjectedData = useMemo(
    () => clauses.map((c) => ({
      ...
      content: injectTokens(
        c.content, formData, selectedClient, selectedProperty,
        initialData.agencyName, financials
      ),
    })),
  ...
  const progress = [
    ...
    { label: "Conditions Financières Définies", done: !!formData.rentAmount,                   step: 4 },
  ]
  ...
  const buildPayload = (): (ContractPayload & { id: number }) | null => {
    ...
    const clausePayload = clauses.map((c, index) => {
      ...
      if (c.clause_id)    return { clause_id: c.clause_id, title: c.title, content, order: index }
      if (serverClauseId) return { contract_clause_id: serverClauseId, title: c.title, content, order: index }
      return { title: c.title, content, order: index }
    })
  ...
  return {
    ...
    clauses, formData, setFormData, financials,
    clausesWithInjectedData, progress, isReadyToSubmit,
    addClauseFromCatalog, addInlineClause,
    updateClause, removeClause, moveClause, duplicateClause,
    buildPayload,
  }
}
