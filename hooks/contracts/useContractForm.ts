...
export function useContractForm(agencyName: string = "", agencyId: number = 0) {
  ...
  const getDefaultClauses = (type: ContractType): ContractClause[] =>
    defaultClauses
      .filter((c) => c.isDefault && (c.type === type || c.type === "both"))
      .map(toContractClause)

  const [clauses, setClauses] = useState<ContractClause[]>(
    () => getDefaultClauses("rental")
  )

  const [formData, setFormData] = useState<FormData>({
    city: "", startDate: "", duration: "12",
    rentAmount: "", deposit: "", cautionMonths: "1",
  ...
  const financials = useMemo(() => {
    ...
    return {
      rentAmount, depositAmount, cautionMonths,
      commissionRate, rentMultiplier,
      cautionTotal, commissionAmount, rentAtSignature, totalAtSignature,
    }
  ...
  const addClauseFromCatalog = (catalogClause: CatalogClause) => {
    const alreadyAdded = clauses.some((c) =>
      c.clause_id
        ...
        : c.id.includes(catalogClause.id) || c.title === catalogClause.title
    )
  ...
  const clausesWithInjectedData = useMemo(
    () => clauses.map((c) => ({
      ...
      content: injectTokens(
        c.content, formData, selectedClient, selectedProperty, agencyName, financials
      ),
    })),
  ...
  const buildPayload = (): ContractPayload | null => {
    ...
    const clausePayload = clauses.map((c) => {
      ...
      if (c.clause_id) return { clause_id: c.clause_id, title: c.title, content }
  ...
  return {
    activeTab, setActiveTab, contractType,
    selectedClient, setSelectedClient,
    selectedProperty, setSelectedProperty,
    showPreview, setShowPreview,
    clauses, formData, setFormData, financials,
    filteredProperties, clausesWithInjectedData,
    progress, isReadyToSubmit,
