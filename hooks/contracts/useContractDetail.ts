...
export function useContractDetail(contractId: string | number) {
  ...
  useEffect(() => {
    ...
    axiosInstance
      .get<{ success: boolean; data: ApiContractDetail }>(
      ...
      .catch((err) => {
        if (!cancelled)
          setError(
            err.response?.data?.message ?? "Impossible de charger le contrat",
          );
      })
      .finally(() => {
...
function normalize(c: ApiContractDetail): ContractEditData {
  ...
  const formData: FormData = {
    agentId: c.agent_id ? String(c.agent_id) : "",
    usageCase:
      c.usage_case ?? (c.type === "rental" ? "habitation" : "compromis"),
    title: c.title ?? "",
    city: c.city,
  ...
  const clauses: ContractClause[] = [...c.contract_clauses]
    ...
    .map((cl) => ({
      ...
      content: cl.content,
      type: c.type,
      source: cl.clause_id ? "agency" : cl.is_custom ? "inline" : "system",
      isModified: false,
