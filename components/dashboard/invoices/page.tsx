...
// components/dashboard/invoices/listing-invoices-page.tsx
...
function getDestinataireName(facture: Facture): string {
  if (facture.destinataire_type === "fournisseur") {
    return facture.fournisseur_nom || "Fournisseur"
  }
...
export default function ListingInvoicesPage({ agencyId }: Props) {
  ...
  const statCards = [
    {
      label: "Total Revenus CFA",
      value: stats ? stats.total_revenue : "—",
      change: stats ? `${stats.factures_non_payee + stats.factures_partiellement_payee} en attente` : "",
      icon: TrendingUp,
    },
    {
      label: "Non Payées CFA",
      value: stats ? stats.non_payee : "—",
      change: stats ? `${stats.factures_non_payee} factures` : "",
      icon: Clock,
    },
    {
      label: "Partiellement Payées CFA",
      value: stats ? stats.partiellement_payee : "—",
      change: stats ? `${stats.factures_partiellement_payee} factures` : "",
      icon: AlertTriangle,
    },
    {
      label: "Soldées CFA",
      value: stats ? stats.soldee : "—",
      change: stats ? `${stats.pourcentage_avancement}% recouvrement` : "",
      icon: CheckCircle,
    },
  ]
  ...
  const handleAddPayment = async () => {
    ...
    if (selectedInvoice && amount > selectedInvoice.montant_restant) {
      setPaymentError(`Le montant ne peut pas dépasser ${formatCurrency(selectedInvoice.montant_restant)}.`)
      return
    }
    ...
    const payload = {
      ...
      date_reglement: paymentDate,
      notes: paymentRef || null,
    }

    // ── Debug ───────────────────────────────────────────────────────────────
    console.log("=== PAYLOAD RÈGLEMENT ===")
    console.log(`POST /api/factures/reglements/${selectedInvoice?.id}`)
    ...
    try {
      ...
      if (response.data && response.data.message) {
        setPaymentError(response.data.message || "Erreur lors de l'enregistrement.")
      ...
      console.error("Erreur réseau:", error.response || error)
  ...
  const handleDeleteInvoice = async () => {
    ...
    try {
      ...
      window.location.reload() // Recharger pour mettre à jour la liste après suppression
    } catch (err) {
      const error = err as any
  ...
  console.log("Factures chargées:", factures) // Debug : afficher les factures dans la console
  return (
                  ...
                  <tr>
                    <td colSpan={8} className="text-center py-12">
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <Button asChild>
                          <Link href="/dashboard/invoices/create">Créer une e-facture</Link>
                        </Button>
                      </div>
                    </td>
                  </tr>
                    ...
                    >
                      ...
                      <td className="py-3 px-4 hidden lg:table-cell">
                        <p className="text-sm text-foreground capitalize">
                          {facture.categorie?.replace(/_/g, " ") || "—"}
                        </p>
                        {facture.sous_categorie && (
                          <p className="text-xs text-muted-foreground">{facture.sous_categorie}</p>
                        )}
                      </td>
                      ...
                      <td className="py-3 px-4 text-right hidden sm:table-cell">
                        <p className="text-sm text-emerald-600 font-medium">
                          {formatCurrency(facture.montant_regle, facture.devise)}
                        </p>
                      </td>
                      <td className="py-3 px-4 text-right hidden sm:table-cell">
                        <p className="text-sm text-foreground">
                          {formatCurrency(facture.montant_restant, facture.devise)}
                        </p>
                      </td>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem
                              ...
                              onClick={() => setInvoiceToDelete(facture)}
                            >
      ...
      <Dialog open={paymentOpen} onOpenChange={setPaymentOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedInvoice &&
                `Facture ${selectedInvoice.numero_facture} — ${getDestinataireName(selectedInvoice)}`}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-5 py-2">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="p-3 rounded-lg bg-muted/50 text-center">
                  ...
                  <p className="font-bold text-foreground text-sm">
                    {formatCurrency(parseFloat(selectedInvoice.montant_ttc), selectedInvoice.devise)}
                  </p>
                </div>
                <div className="p-3 rounded-lg bg-muted/50 text-center">
                  <p className="text-xs text-muted-foreground mb-1">Déjà payé</p>
                  <p className="font-bold text-emerald-600 text-sm">
                    {formatCurrency(selectedInvoice.montant_regle, selectedInvoice.devise)}
                  </p>
                </div>
                <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-center">
                  <p className="text-xs text-amber-600 mb-1">Restant</p>
                  <p className="font-bold text-amber-600 text-sm">
                    {formatCurrency(selectedInvoice.montant_restant, selectedInvoice.devise)}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="pay-amount">
                  Montant à régler <span className="text-destructive">*</span>
                </Label>
                <Input
                  id="pay-amount"
                  type="number"
                  min="0.01"
                  step="1"
                  max={selectedInvoice.montant_restant}
                  placeholder="0"
                  value={paymentAmount}
                  onChange={(e) => {
                    setPaymentError("")
                    const val = parseFloat(e.target.value) || 0
                    // Bloquer si supérieur au montant restant
                    if (val > selectedInvoice.montant_restant) {
                      setPaymentAmount(selectedInvoice.montant_restant.toString())
                    } else {
                ...
                {paymentAmount && parseFloat(paymentAmount) > 0 && (
                  parseFloat(paymentAmount) > selectedInvoice.montant_restant ? (
                    <p className="text-xs text-destructive flex items-center gap-1">
                      ...
                      Maximum autorisé : {formatCurrency(selectedInvoice.montant_restant, selectedInvoice.devise)}
                    </p>
                  ) : (
                    <p className="text-xs text-muted-foreground">
                      Restant après règlement :{" "}
                      {formatCurrency(
                        selectedInvoice.montant_restant - parseFloat(paymentAmount),
                        selectedInvoice.devise
                      )}
                    </p>
                  )
                )}
                ...
                <div className="grid grid-cols-3 gap-2">
                  {paymentMethods.map((m) => (
                    <button
                      ...
                      className={`flex flex-col items-center gap-1.5 p-3 rounded-lg border-2 text-xs font-medium transition-all cursor-pointer ${selectedPaymentMethod === m.value
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="pay-ref"
                  placeholder="ex: numéro de virement, remarque..."
                  value={paymentRef}
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2">
              ...
              <span className="block font-medium text-foreground">
                La suppression de la facture <strong>{invoiceToDelete?.numero_facture}</strong> entraînera également la suppression de :
              </span>
              <ul className="list-disc list-inside text-sm space-y-1">
                <li>Tous les <strong>règlements</strong> associés à cette facture</li>
              </ul>
