...
import {
  ...
  Eye, Pencil, Send, CheckCircle, Clock, TrendingUp,
} from "lucide-react"
...
const getProgress = (status: ContractStatus): number => {
  const map: Record<ContractStatus, number> = {
    draft: 25, revision: 50, sent: 90, approved: 75,
...
function ContractStats({ stats }: { stats: ContractsStats }) {
  const items = [
    { label: "Contrats Totaux",         value: stats.total.toString(),   icon: FileText,    color: "text-blue-500",   bg: "bg-blue-500/10" },
    { label: "En Attente de Signature", value: stats.pending.toString(), icon: Clock,       color: "text-amber-500",  bg: "bg-amber-500/10" },
    { label: "Signés",                  value: stats.signed.toString(),  icon: CheckCircle, color: "text-green-500",  bg: "bg-green-500/10" },
    { label: "Valeur Totale",           value: formatAmount(stats.value),icon: TrendingUp,  color: "text-purple-500", bg: "bg-purple-500/10" },
  ]
  return (
...
interface ActionState { loading: boolean; error: string | null }

function ContractFilters({ search, onSearchChange, statusFilter, onStatusChange, typeFilter, onTypeChange }: ContractFiltersProps) {
  return (
    <Card>
      <CardContent className="p-4">
...
function ContractTable({ contracts }: { contracts: Contract[] }) {
  const [sendLoading, setSendLoading] = useState<Record<number, ActionState>>({})
    ...
    return (
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">Modifiez vos filtres ou créez un nouveau contrat.</p>
        </CardContent>
      </Card>
    )
  }
  ...
  const handleSendContract = async (id: number) => {
    setSendLoading((prev) => ({ ...prev, [id]: { loading: true, error: null } }))
    try {
      await axiosInstance.patch(`/api/contracts/send/${id}`, {})
      setSendLoading((prev) => ({ ...prev, [id]: { loading: false, error: null } }))
      toast.success("Contrat envoyé pour signature !")
      window.location.reload()
    } catch (err: any) {
      const message = err?.response?.data?.message ?? "Erreur lors de l'envoi"
      setSendLoading((prev) => ({ ...prev, [id]: { loading: false, error: message } }))
      toast.error(message)
    }
  }
            ...
            <tbody>
              {contracts.map((contract) => (
                <tr key={contract.id} className="border-b border-border last:border-0 hover:bg-muted/50 transition-colors">
                  ...
                  <td className="py-4 px-4">
                    <p className="font-medium text-foreground truncate max-w-[200px]">{contract.bien.title}</p>
                    <p className="text-sm text-muted-foreground">{contract.client.prenom} {contract.client.nom}</p>
                  </td>
                  ...
                  <td className="py-4 px-4 hidden xl:table-cell">
                    <div className="w-[100px]">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">{getProgress(contract.status)}% complété</p>
                    </div>
                  </td>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem asChild>
                          <Link href={`/dashboard/contracts/${contract.id}`}>
                            <Eye className="mr-2 h-4 w-4" />Voir le Contrat
                          </Link>
                        </DropdownMenuItem>
                        <DropdownMenuItem asChild>
                        ...
                        {/* ── Un seul bouton PDF ── */}
                        <DropdownMenuItem
                          onSelect={() => openPdf(`/api/contracts/${contract.id}/pdf`, contract.id)}
                        ...
                        <DropdownMenuItem
                          onSelect={() => handleSendContract(contract.id)}
                          disabled={sendLoading[contract.id]?.loading}
                        >
                          <Send className="mr-2 h-4 w-4" />Envoyer pour Signature
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem className="text-destructive">Annuler le Contrat</DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </td>
                </tr>
...
export default function ListingContractsPage({ contracts, stats, loading = false }: ListingContractsPageProps) {
  ...
  const filteredContracts = contracts.filter((contract) => {
    ...
    const matchSearch =
      ...
      `${contract.client.prenom} ${contract.client.nom}`.toLowerCase().includes(search.toLowerCase()) ||
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[...Array(4)].map((_, i) => (
            <Card key={i}><CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="space-y-2"><Skeleton className="h-3 w-32" /><Skeleton className="h-7 w-16" /></div>
          ...
          <table className="w-full">
            <thead><tr className="border-b border-border">
              {["N° Contrat","Bien / Client","Type","Montant","Ville","Statut","Progression",""].map((h,i) => (
                <th key={i} className="py-4 px-4 text-left"><Skeleton className="h-3 w-20" /></th>
              ))}
            </tr></thead>
            <tbody>{[...Array(6)].map((_, i) => (
              <tr key={i} className="border-b border-border last:border-0">
                <td className="py-4 px-4"><div className="flex items-center gap-3"><Skeleton className="h-10 w-10 rounded-lg shrink-0" /><div className="space-y-1.5"><Skeleton className="h-4 w-28" /><Skeleton className="h-3 w-20" /></div></div></td>
                <td className="py-4 px-4"><div className="space-y-1.5"><Skeleton className="h-4 w-40" /><Skeleton className="h-3 w-28" /></div></td>
                <td className="py-4 px-4 hidden md:table-cell"><Skeleton className="h-5 w-16 rounded-full" /></td>
                <td className="py-4 px-4 hidden lg:table-cell"><Skeleton className="h-4 w-24" /></td>
                <td className="py-4 px-4 hidden lg:table-cell"><Skeleton className="h-4 w-20" /></td>
                <td className="py-4 px-4"><Skeleton className="h-5 w-24 rounded-full" /></td>
                <td className="py-4 px-4 hidden xl:table-cell"><div className="w-[100px] space-y-1"><Skeleton className="h-2 w-full rounded-full" /><Skeleton className="h-3 w-16" /></div></td>
                <td className="py-4 px-4 text-right"><Skeleton className="h-8 w-8 rounded-md ml-auto" /></td>
              </tr>
            ))}</tbody>
          </table>
