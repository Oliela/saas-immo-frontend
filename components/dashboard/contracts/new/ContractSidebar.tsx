...
import { Eye, Send, Save, Check, User, AlertCircle } from "lucide-react"
...
export function ContractSidebar({
  ...
  clauseCount, progress, isReadyToSubmit,
  submitError, isEdit = false,
  onPreview, onSubmit, onSaveDraft,
}: Props) {
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <Button className="w-full" onClick={onSubmit} disabled={!isReadyToSubmit}>
          <Send className="mr-2 h-4 w-4" />
          {isEdit ? "Enregistrer les Modifications" : "Générer et Envoyer le Contrat"}
        </Button>

        <Button variant="outline" className="w-full bg-transparent" onClick={onSaveDraft}>
          <Save className="mr-2 h-4 w-4" />
          {isEdit ? "Sauvegarder" : "Enregistrer en Brouillon"}
        </Button>
      </div>
    </div>
  )
