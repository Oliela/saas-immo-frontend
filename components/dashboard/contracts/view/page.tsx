...
export default function ContractViewPage({ contract }: ContractViewPageProps) {
    ...
    const paymentFrequencyMultiplier: Record<string, number> = {
        ...
        <div className="space-y-6">
            ...
            <div className="grid gap-6 lg:grid-cols-3">
                    ...
                    <Tabs defaultValue="details">
                        <TabsList>
                            ...
                            <TabsTrigger value="clauses">
                                ...
                                Clauses ({contract_clauses.length})
                            </TabsTrigger>
                        ...
                        <TabsContent value="details" className="space-y-6 mt-6">
                                ...
                                <CardContent className="space-y-3">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-medium text-foreground">
                                            {contract.commission}% — {formatAmount(commissionAmount.toString())}
                                        </span>
                                    </div>
                                    <Separator />
                                    {contract.type === "rental" && (
                                        <>
                                            <div className="flex justify-between">
                                                <span className="text-muted-foreground">
                                                    Caution ({contract.cautionMonths} mois × {formatAmount(contract.deposit)})
                                                </span>
                                                <span className="font-medium text-foreground">
                                                    {formatAmount(cautionTotal.toString())}
                                                </span>
                                            </div>
                                            <Separator />
                                            <div className="flex justify-between">
                                                <span className="text-muted-foreground">
                                                    ...
                                                    <span className="text-xs text-muted-foreground">
                                                        ({rentMultiplier} mois — {paymentFrequencyLabel[contract.payment_frequency]})
                                                    </span>
                                                </span>
                                                <span className="font-medium text-foreground">
                                                    {formatAmount(rentAtSignature.toString())}
                                                </span>
                                            </div>
                                            <Separator />
                                            <div className="flex justify-between">
                                                ...
                                                <span className="font-medium text-foreground">
                                                    {paymentFrequencyLabel[contract.payment_frequency]}
                                                </span>
                                            </div>
                                            <Separator />
                                        </>
                                    ...
                                    <div className="flex justify-between bg-muted/50 rounded-lg p-3">
                                        ...
                                        <span className="font-bold text-foreground text-lg">
                                            {formatAmount(totalSignature.toString())}
                                        </span>
                                    </div>
                                    {contract.type === "rental" && (
                            ...
                            <Card>
                                <CardHeader>
                                    <CardTitle className="flex items-center gap-2 text-lg">
                                        <Calendar className="h-5 w-5" />Dates Clés
                                    </CardTitle>
                                </CardHeader>
                                <CardContent className="space-y-3">
                                    <div className="flex justify-between">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-medium text-foreground">
                                            {contract.sent_at ? new Date(contract.sent_at).toLocaleDateString("fr-FR") : "—"}
                                        </span>
                                    </div>
                                    <Separator />
                                    <div className="flex justify-between">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-medium text-foreground">
                                            {contract.expired_at ? new Date(contract.expired_at).toLocaleDateString("fr-FR") : "—"}
                                        </span>
                                    </div>
                                    <Separator />
                                    <div className="flex justify-between">
                        ...
                        <TabsContent value="clauses" className="mt-6">
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>
                                        {contract_clauses.length} clause{contract_clauses.length > 1 ? "s" : ""} dans ce contrat
                                    </CardDescription>
                                </CardHeader>
                                        ...
                                        .map((clause, index) => (
                                            <div key={clause.id} className="p-4 rounded-lg border border-border">
                                                <div className="flex items-center gap-2 mb-2">
                                                    ...
                                                    {clause.is_custom === 1 && (
                                                        <Badge variant="secondary" className="text-xs">Personnalisée</Badge>
                                                    )}
                                                </div>
                        ...
                        <TabsContent value="history" className="mt-6">
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>
                                        Progression du contrat à travers les étapes du workflow
                                    </CardDescription>
                                </CardHeader>
                                <CardContent>
                                    {isTerminal ? (
                                        ...
                                        <div className="space-y-0">
                                            {WORKFLOW_STEPS.map((step) => {
                                                ...
                                                // ✅ isCompleted inclut l'étape actuelle (<=)
                                                const isCompleted = !isCancelledStep
                                                    && normalIndex <= normalCurrentIndex
                                                    && !isContractCancelled
                                                ...
                                                const matchingEvent = histories?.find(
                                                    h => h.action === step.status || (h.action === "created" && step.status === "draft")
                                                )

                                                const StepIcon = step.icon

                                                const visibleSteps = WORKFLOW_STEPS.filter(
                                                    s => s.status !== "cancelled" || isContractCancelled
                                                )
                                                const isLastVisible = visibleSteps[visibleSteps.length - 1].status === step.status

                                                const dotStyle = (): React.CSSProperties => {
                                                    if (isCancelledStep && isCurrent)
                                                        return { backgroundColor: '#ef4444', borderColor: '#ef4444', color: 'white' }
                                                    if (isCompleted)
                                                        return { backgroundColor: '#22c55e', borderColor: '#22c55e', color: 'white' }
                                                    return {}
                                                }

                                                const lineStyle = (): React.CSSProperties => {
                                                    if (isCompleted) return { backgroundColor: '#22c55e' }
                                                    return {}
                                                }
                                                    ...
                                                    <div key={step.status} className="flex gap-4 pb-6 last:pb-0">
                                                        <div className="flex flex-col items-center">
                                                            <div
                                                                ...
                                                                className={cn(
                                                                    ...
                                                                    !isCompleted && !(isCancelledStep && isCurrent)
                                                        ...
                                                        <div className="flex-1 pb-2 pt-1">
                                                            <div className="flex items-center gap-2 flex-wrap">
                                                                ...
                                                                {/* ✅ "Complété" a priorité — s'affiche pour toutes les étapes isCompleted */}
                                                                {isCompleted && (
                                                                    <Badge
                                                                        variant="outline"
                                                                        className="text-xs"
                                                                ...
                                                                {/* "Étape actuelle" uniquement si PAS complété */}
                                                                {isCurrent && !isCancelledStep && !isCompleted && (
                                                            ...
                                                            {matchingEvent && (
                                                                <div className="mt-2 flex items-center gap-2">
                                                                    ...
                                                                    <span className="text-xs text-muted-foreground">
                                                                        {new Date(matchingEvent.created_at).toLocaleDateString("fr-FR", {
                                                                            day: "numeric", month: "long", year: "numeric",
                                                                        })}
                                                                    </span>
                ...
                <div className="space-y-6">
                        ...
                        <CardContent>
                            <div className="flex items-center gap-3 mb-4">
                                <Avatar className="h-12 w-12">
                                    <AvatarFallback>{getInitials(client.prenom, client.nom)}</AvatarFallback>
                                </Avatar>
                                <div>
                            ...
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-medium text-foreground">{client.employer}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-muted-foreground">Revenu Mensuel</span>
                                    <span className="font-medium text-foreground">{formatAmount(client.monthly_income)}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-muted-foreground">Budget</span>
                                    <span className="font-medium text-foreground">{formatAmount(client.monthly_budget)}</span>
                                </div>
                            </div>
                            <Separator className="my-3" />
                            <p className="text-xs font-medium text-muted-foreground mb-2">Documents fournis</p>
                            <div className="flex flex-wrap gap-1">
                                {client.id_document === "true" && (
                                    <Badge variant="secondary" className="text-xs">
                                        <CheckCircle className="h-3 w-3 mr-1" />Pièce d'identité
                                    </Badge>
                                )}
                                {client.income_proof === "true" && (
                                    <Badge variant="secondary" className="text-xs">
                                        <CheckCircle className="h-3 w-3 mr-1" />Justif. revenus
                                    </Badge>
                                )}
                                {client.bank_statement === "true" && (
                                    <Badge variant="secondary" className="text-xs">
                                        <CheckCircle className="h-3 w-3 mr-1" />Relevé bancaire
                                    </Badge>
                                )}
                                {client.recommendation_letter === "true" && (
                                    <Badge variant="secondary" className="text-xs">
                                        <CheckCircle className="h-3 w-3 mr-1" />Lettre de recommandation
                                    </Badge>
                                )}
                            </div>
                        </CardContent>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-lg">Infos Contrat</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-2 text-sm">
                            <div className="flex justify-between">
                                <span className="text-muted-foreground">Ville</span>
                                <span className="font-medium text-foreground">{contract.city}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-muted-foreground">N° Contrat</span>
                                <span className="font-medium text-foreground text-xs">{contract.contract_number}</span>
                            </div>
                            <div className="flex justify-between">
