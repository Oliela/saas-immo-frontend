...
import {
  ...
  MoreHorizontal, CheckCircle, XCircle, AlertCircle, Loader2, Trash2,
} from "lucide-react"
...
export default function VisitsAllList({ visits = [], loading }: VisitsAllListProps) {
  ...
  const [loadingAction, setLoadingAction] = useState<"reminder" | "done" | "cancel" | "confirmed" | "delete" | null>(null)
  const [visitToDelete, setVisitToDelete] = useState<Reservation | null>(null)
  ...
  const handleDeleteVisit = async () => {
    ...
    setLoadingId(visitToDelete.id); setLoadingAction("delete")
    try {
      await axiosInstance.delete(`/api/visit-reservations/${visitToDelete.id}`)
      toast.success("Visite supprimée avec succès.")
      window.location.reload()
    } catch (err: any) {
      toast.error(err?.response?.data?.message ?? "Erreur lors de la suppression.")
    } finally {
      setLoadingId(null); setLoadingAction(null); setVisitToDelete(null)
    }
  }
  ...
  return (
    <>
                  ...
                  >
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuSeparator />
                        <DropdownMenuItem className="text-destructive" onClick={() => setVisitToDelete(reservation)}>
      ...
      <AlertDialog open={visitToDelete !== null} onOpenChange={(open) => !open && setVisitToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              <AlertCircle className="h-5 w-5 text-destructive" />Supprimer cette visite ?
            </AlertDialogTitle>
            <AlertDialogDescription>
              Cette action est irréversible. La visite prévue le{" "}
              {visitToDelete && new Date(visitToDelete.visit_schedule.visit_date).toLocaleDateString("fr-FR")} pour{" "}
              <strong>{visitToDelete?.client.prenom} {visitToDelete?.client.nom}</strong> sera définitivement supprimée.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
