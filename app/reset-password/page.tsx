...
function ResetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setError("")
    if (!allRequirementsMet) { setError("Veuillez respecter toutes les exigences du mot de passe."); return }
    ...
    return (
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription>Votre mot de passe a été modifié. Vous pouvez maintenant vous connecter.</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input id="password" type={showPassword ? "text" : "password"} placeholder="Saisissez un nouveau mot de passe"
                value={password} onChange={(e) => setPassword(e.target.value)} required disabled={isLoading} className="pr-10" />
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input id="confirmPassword" type={showConfirmPassword ? "text" : "password"} placeholder="Confirmez le nouveau mot de passe"
          ...
          {error && (
            <div className="flex items-center gap-2 rounded-lg bg-destructive/10 p-3 text-sm text-destructive">
              ...
              {error.includes("nouveau lien") && (
                <Link href="/forgot-password" className="ml-1 underline font-medium">Cliquez ici</Link>
              )}
            </div>
          )}

          <Button type="submit" className="w-full" size="lg" disabled={isLoading || !allRequirementsMet || !passwordsMatch}>
            {isLoading ? (
              <>
...
// ← Page principale avec Suspense qui wrap useSearchParams
